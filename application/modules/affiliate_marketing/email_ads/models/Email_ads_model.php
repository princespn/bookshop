 ...
 * @filesource
 */
class Email_ads_model extends Affiliate_marketing_model
{
	protected $id = 'id';
	...
	public function install($id = '')
	{
		...
		$sql = "CREATE TABLE IF NOT EXISTS " . $this->db->dbprefix(config_item('module_table')) . " (
                  ...
                  `email_ad_title` varchar(255) COLLATE utf8_unicode_ci NOT NULL DEFAULT '',
                  `email_ad_body` longtext COLLATE utf8_unicode_ci NOT NULL,
	...
	function parse_tags($data = array())
	{
		foreach ($data as $k => $v)
		{
			$data[$k]['email_ad_title'] = parse_string($v['email_ad_title'], session_tags('tools/email_ads/' . $v['id']));
			$data[$k]['email_ad_body'] = parse_string($v['email_ad_body'], session_tags('tools/email_ads/' .  $v['id']));
		}
...
/* End of file Email_ads_model.php */
/* Location: ./modules/affiliate_marketing/email_ads/models/Email_ads_model.php */