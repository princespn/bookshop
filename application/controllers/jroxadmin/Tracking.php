...
class Tracking extends Admin_Controller
{
	...
	public function view()
	{
		...
		$this->data['rows'] = $this->track->get_rows($this->data['page_options']);
		...
		//run the page
		$this->load->page('promotions/' . TPL_ADMIN_TRACKING_VIEW, $this->data);
	}
	...
	public function create()
	{
		...
		$this->data['row'] = set_default_form_values(array(TBL_TRACKING));

		if ($this->input->post())
		{
			//check if the form submitted is correct
			$row = $this->dbv->validate(TBL_TRACKING, TBL_TRACKING, $this->input->post());

			if (!empty($row['success']))
			{
				$row = $this->dbv->create(TBL_TRACKING, $row['data']);
		...
		$this->load->page('promotions/' . TPL_ADMIN_TRACKING_CREATE, $this->data);
	}
	...
	public function update()
	{
		...
		if ($this->input->post())
		{
			...
			if (!empty($row['success']))
			{
				$row = $this->dbv->update(TBL_TRACKING, 'tracking_id', $row['data']);
		...
		$this->data['row'] = $this->track->get_details($this->data['id'], FALSE, FALSE);
		...
		$this->load->page('promotions/' . TPL_ADMIN_TRACKING_UPDATE, $this->data);
	}
	...
	public function delete()
	{
		...
		$row = $this->dbv->delete(TBL_TRACKING, 'tracking_id', $id);
...
/* Location: ./application/controllers/admin/Tracking.php */