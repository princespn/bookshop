 ...
 * @filesource
 */
class Top_product_views_model extends Reports_model
{
	/**
	 * Install module
	 *
	...
	protected function run_query($days = '')
	{
		$sql = 'SELECT p.product_id, p.product_views as amount,
					n.product_id AS name, n.product_name
				FROM ' . $this->db->dbprefix(TBL_PRODUCTS) . ' p
				LEFT JOIN ' . $this->db->dbprefix(TBL_PRODUCTS_NAME) . ' n
					ON p.product_id = n.product_id
					AND n.language_id = \'' . sess('default_lang_id') . '\'
...
/* Location: ./application/modules/admin_reporting/Month_top_product_views/models/Month_top_product_views_model.php */