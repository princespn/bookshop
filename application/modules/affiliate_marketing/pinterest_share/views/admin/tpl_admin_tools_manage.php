...
<div class="row">
	<div class="col-md-4">
		<?= generate_sub_headline('manage_link', 'fa-edit', '') ?>
	</div>
	<div class="col-md-8 text-right">
		...
		<a href="<?= admin_url(CONTROLLER_CLASS . '/view_rows/0/?module_id=' . $module_id) ?>"
		   class="btn btn-primary"><?= i('fa fa-search') ?> <span
				class="hidden-xs"><?= lang('view_links') ?></span></a>
		<?php if (CONTROLLER_FUNCTION == 'update'): ?>
...
<div class="box-info">
	...
	<div class="tab-content">
		<div class="tab-pane active" id="config">
			<h3 class="text-capitalize"><?= lang('link_details') ?></h3>
			<hr/>
			...
			<hr/>
			<div class="form-group">
				<?= lang('link_text', 'link_text', array('class' => 'col-md-3 control-label')) ?>
				<div class="col-md-5">
					<?= form_input('link_text', set_value('link_text', $row['link_text']), 'class="' . css_error('link_text') . ' form-control required"') ?>
				</div>
			</div>
			<hr/>
			<div class="form-group">
				<?= lang('image_file_name', 'image_file_name', array('class' => 'col-md-3 control-label')) ?>
				<div class="col-md-5">
					<div class="input-group">
						<?= form_input('image_file_name', set_value('image_file_name', $row['image_file_name']), 'id="1" class="' . css_error('image_file_name') . ' form-control required"') ?>
						<span class="input-group-addon"> <a class="iframe"
						                                    href="<?= site_url() ?>filemanager/dialog.php?type=1&akey=<?= $file_manager_key ?>&field_id=1"><?= i('fa fa-upload') ?> <?= lang('update_image') ?></a></span>
					</div>
				</div>
			</div>
			<hr/>
