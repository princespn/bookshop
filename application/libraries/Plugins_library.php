...
class Plugins_library
{
	...
	public function find_plugins()
	{
		...
		if ($plugins != FALSE)
		{
			...
			{
				$name = substr(strtolower(trim($name)), 0, -1);
				...
				if (!isset(self::$plugins_pool[ $name ]) AND !stripos($name, "."))
				{
					...
					if (file_exists($this->plugins_dir . $name . "/" . $name . ".php"))
					{
						...
						$this->get_plugin_headers($name);
						self::$plugins_active[ $name ] = $name;
					}
					else
					{
	...
	public static function instance()
	{
		if (!self::$instance)
		{
			self::$instance = new Plugins_library();
		}
...
{
	return Plugins_library::instance()->add_action($name, $function, $priority);
}
...
{
	return Plugins_library::instance()->do_action($name, $arguments);
}
...
{
	return Plugins_library::instance()->remove_action($name, $function, $priority);
}
...
{
	return Plugins_library::instance()->action_exists($name);
}
...
{
	Plugins_library::instance()->set_plugin_dir($directory);
}
...
{
	return Plugins_library::instance()->activate_plugin($name);
}
...
{
	return Plugins_library::instance()->deactivate_plugin($name);
}
...
{
	return Plugins_library::instance()->print_plugins();
}
...
{
	return count(Plugins_library::$plugins_pool);
}
...
{
	return count(Plugins_library::$plugins_active);
}
...
{
	if (is_array(Plugins_library::$errors))
	{
		foreach (Plugins_library::$errors AS $k => $error)
		{
			echo $error . "\n\r";
		}
	}
...
{
	if (is_array(Plugins_library::$messages))
	{
		foreach (Plugins_library::$messages AS $k => $message)
		{
			echo $message . "\n\r";
		}
	}
