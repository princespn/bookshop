 ...
 * @filesource
 */
class Affiliate_traffic extends Admin_Controller
{
	...
	public function view()
	{
		...
		$this->data['rows'] = $this->aff->get_referrals($this->data['page_options']);
		...
		//run the page
		$this->load->page('affiliate/' . TPL_ADMIN_AFFILIATE_TRAFFIC_VIEW, $this->data);
	}
	...
	public function delete()
	{
		...
		$row = $this->dbv->delete(TBL_AFFILIATE_TRAFFIC, 'traffic_id', $id);
	...
	public function block_traffic()
	{
		if ($this->input->post())
		{
			...
			//set the session flash and redirect the page
			redirect_flashdata(ADMIN_ROUTE . '/' . strtolower(__CLASS__) . '/' . __FUNCTION__, $row[ 'msg_text' ]);
		}

		$this->load->page('affiliate/' . TPL_ADMIN_AFFILIATE_BLOCK_TRAFFIC, $this->data);
	}
}
