				...
				<?php foreach ($row['values'] as $v): ?>
					<?php if ($v['type'] != 'hidden'): ?>
						<div class="form-group">
							...
							<div class="col-lg-5">
								<?php if ($v['key'] == 'module_mailing_lists_aweber_app_id'): ?>
									<?php if (!empty($v['value'])): ?>
										<div class="input-group">
											<input type="text" name="module_mailing_lists_aweber_app_id"
											       class="form-control required"
											       value="<?= $v['value'] ?>"/>
											<span class="input-group-btn">
                                                    <a href="https://auth.aweber.com/1.0/oauth/authorize_app/<?= $v['value'] ?>"
                                                       class="btn btn-default"
                                                       target="_blank"><?= lang('authorize_app') ?></a>
                                                </span>
										</div>
									<?php else: ?>
