 ...
 * @filesource
 */
class Month_top_affiliate_commissions_model extends Reports_model
{
	/**
	 * Install module
	 *
	...
	{
		$html = $this->init_data($data, $limit, 'top');
	...
	{
		$sql = 'SELECT p.member_id, SUM(p.commission_amount) as amount,
					n.fname, n.lname, n.username AS name,
					e.profile_photo
				FROM ' . $this->db->dbprefix(TBL_AFFILIATE_COMMISSIONS) . ' p
				LEFT JOIN ' . $this->db->dbprefix(TBL_MEMBERS) . ' n
					ON p.member_id = n.member_id
				LEFT JOIN ' . $this->db->dbprefix(TBL_MEMBERS_PROFILES) . ' e
					ON n.member_id = e.member_id
				   WHERE p.date BETWEEN \'' . current_date('Y', $m,'', $y) . '-' . current_date('m', $m) . '-01 00:00:00\'
                    AND \'' . current_date('Y', $m, '', $y) . '-' . current_date('m', $m) . '-' . $days . ' 23:59:59\' 
				  GROUP BY member_id
				  ORDER BY amount DESC LIMIT ' . config_option('query_limit');
...
/* End of file month_top_affiliate_commissions_model.php */
/* Location: ./application/modules/admin_reporting/month_top_affiliate_commissions/models/month_top_affiliate_commissions_model.php */