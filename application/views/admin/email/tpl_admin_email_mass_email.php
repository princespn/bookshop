...
<div class="row">
	<div class="col-md-4">
		<?= generate_sub_headline('send_mass_email', 'fa-edit', '') ?>
	</div>
...
<div class="box-info">
	...
	<div class="tab-content">
		<div class="tab-pane fade in active" id="html">
			<h3 class="text-capitalize"><?= lang('send_to_mailing_list') ?></h3>
			<hr/>
			<div class="form-group">
				<label class="col-md-2 control-label"><?= lang('to') ?></label>

				<div class="col-md-3">
					<div class="form-control"><?=$row['list_name']?></div>
					<?= form_hidden('list_id', set_value('list_id', $id)) ?>
				</div>
			...
			<div class="form-group">
				...
				<div class="col-md-3">
					<?= form_dropdown('', merge_fields('mass_email'), '', 'class="text-lowercase form-control" id="merge_fields"') ?>
				</div>
			</div>
			<hr/>
			<div class="form-group">
	...
	$("select#load_templates").change(function () {
		$("select#load_templates option:selected").each(function () {
			if ($(this).attr("value") != "0") {
				id = $(this).attr("value");
				location.href = '<?=admin_url('email_send/mailing_list/' . $id . '?template=')?>' + id;
			}
		});
	}).change();
