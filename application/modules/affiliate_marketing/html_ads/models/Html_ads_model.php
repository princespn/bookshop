 ...
 * @filesource
 */
class Html_ads_model extends Affiliate_marketing_model
{
	protected $id = 'id';
	...
	public function install($id = '')
	{
		...
		$sql = "CREATE TABLE IF NOT EXISTS " . $this->db->dbprefix(config_item('module_table')) . " (
                  ...
                  `html_ad_title` varchar(255) COLLATE utf8_unicode_ci NOT NULL DEFAULT '',
                  `html_ad_body` longtext COLLATE utf8_unicode_ci NOT NULL,
	...
	public function generate_module($data = array(), $member_id = '')
	{
		...
		$row = array('title'         => $this->config->item('module_title'), //title of the report
		             ...
		             'rendered_html' => render_template($data, 'members'), //rendered html for code
		);
	...
	protected function run_query($data = array())
	{
		...
		//parse tags
		$row['values'] = $this->parse_tags($row['values']);

		return empty($row) ? FALSE : $row;
	}

	function parse_tags($data = array())
	{
		foreach ($data as $k => $v)
		{
			$data[$k]['html_ad_title'] = parse_string($v['html_ad_title'], session_tags('tools/html_ads/' . $v['id']));
			$data[$k]['html_ad_body'] = parse_string($v['html_ad_body'], session_tags('tools/html_ads/'  . $v['id']));
		}
	...
	public function validate_record($data = array())
	{
		...
		$row = $this->dbv->validate(config_item('module_table'), $required, $data, FALSE);

		return $row;
	}
}

/* End of file Html_ads_model.php */
/* Location: ./modules/affiliate_marketing/html_ads/models/Html_ads_model.php */