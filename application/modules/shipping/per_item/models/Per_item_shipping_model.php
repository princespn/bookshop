 ...
 * @filesource
 */
class Per_item_shipping_model extends Shipping_model
{
	...
	public function install($id = '')
	{
		...
		$config = array(
			'settings_key'        => 'module_shipping_per_item_charge_shipping_tax',
			'settings_value'      => '0',
		...
		$config = array(
			'settings_key'        => 'module_shipping_per_item_tax_class',
			'settings_value'      => '1',
			'settings_module'     => 'shipping',
			'settings_type'       => 'dropdown',
			'settings_group'      => $id,
	...
	public function generate_rates($data = array(), $cart = array())
	{
		...
		if (!empty($zone_rate))
		{
			...
			foreach ($cart['items'] as $v)
			{
				...
				{
					$item = $zone_rate['amount'] + $v['shipping_cost'];
					$zone_rate['shipping_amount'] += $item;

					$zone_rate['shipping_amount'] *= $v['quantity'];
				}
			}

			//add some taxes and handling...
			$zone_rate['shipping_taxes'] = calc_shipping_taxes($zone_rate['shipping_amount'], 'per_item');
...
/* End of file Per_item_shipping_model.php */
/* Location: ./application/modules/shipping/per_item/models/Per_item_shipping_model.php */