 ...
 * @filesource
 */
class Import_products_model extends Data_import_model
{
	...
	{
		$config = array(
			'settings_key'        => 'module_data_import_products_use_server_path',
			'settings_value'      => '0',
		...
		$config = array(
			'settings_key'        => 'module_data_import_products_server_file_path',
		...
		$config = array(
			'settings_key'        => 'module_data_import_products_delimiter',
		...
		$config = array(
			'settings_key'        => 'module_data_import_products_generate_new_ids',
			'settings_value'      => '1',
			'settings_module'     => 'data_import',
			'settings_type'       => 'dropdown',
			'settings_group'      => $id,
		...
		$config = array(
			'settings_key'        => 'module_data_import_products_first_row_column_names',
			'settings_value'      => '1',
			'settings_module'     => 'data_import',
			'settings_type'       => 'dropdown',
			'settings_group'      => $id,
	...
	public function validate_import_module($data = array())
	{
		...
		if (!file_exists($data['module_data_import_products_server_file_path']))
		{
	...
	public function generate_fields()
	{
		...
		$csv = array_map('read_csv', file(config_option('module_data_import_products_server_file_path')));
	...
	public function generate_field_names()
	{
		...
		foreach ($this->config->item('module_import_tables') as $v)
		{
			...
			foreach ($fields as $f)
			{
				if (config_option('module_data_import_products_generate_new_ids') == 1)
				{
					if ($f == 'product_id')
					{
						continue;
	...
	public function do_import($data = array())
	{
		...
		//let's read the file now...
		$lines = array_map('str_getcsv', file(config_option('module_data_import_products_server_file_path')));
		...
		if (!empty($lines))
		{
			...
			foreach ($lines as $a => $b)
			{
				...
				if ($a == 0)
				{
					if (config_option('module_data_import_products_first_row_column_names') == 1)
					{
						continue;
					}
				}
	...
	public function import_fields($data = array())
	{
		...
		//set dates
		$data[TBL_PRODUCTS]['date_added'] = get_time(now(), TRUE);
		$data[TBL_PRODUCTS]['date_available'] = get_time(now(), TRUE);
		$data[TBL_PRODUCTS]['date_expires'] = get_time(now() + 31536000 , TRUE);

		$prod = $this->dbv->create(TBL_PRODUCTS, $data[ TBL_PRODUCTS ]);
		...
		foreach ($data as $k => $v)
		{
			...
			switch ($k)
			{
				...
				case TBL_PRODUCTS_TO_ATTRIBUTES:

					if (!empty($v['attribute_id']))
					{
						$v['product_attributes'] = $v['attribute_id'];
						$row = $this->att->update_product_attributes($v['product_id'], $v);
					}

					break;

				case TBL_PRODUCTS_TO_TAGS:

					if (!empty($v['tag_id']))
					{
						$row = $this->tag->add_import_tags($v['product_id'], $v['tag_id']);
					}

					break;

				case TBL_PRODUCTS_TO_CATEGORIES:

					if (!empty($v['category_id']))
					{
						$v['product_categories'] = $v['category_id'];
						$row = $this->prod->update_product_categories($v);
					}
		...
		//log it!
		$this->dbv->rec(array('method' => __METHOD__, 'msg' => lang('imported_successfully')));
					...
					foreach ($fields[$k] as $f)
					{
						...
						switch ($f->name)
						{
							case 'product_type':
								...
								$rule .= '|required|in_list[' . implode(',', config_option('product_types')) . ']';

								break;

							case 'product_id':

								if (config_option('module_data_import_products_generate_new_ids') == '0')
								{
									$rule .= '|check_product_id';

									$custom_lang = array(
										'check_product_id' => '%s ' . $v['product_id'] . ' ' . lang('already_exists'),
									);
								}
...
/* End of file Import_products_model.php */
/* Location: ./application/modules/data_import/products/models/Import_products_model.php */