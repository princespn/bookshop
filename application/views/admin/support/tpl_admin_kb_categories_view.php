...
<?= form_open(admin_url(CONTROLLER_CLASS . '/mass_update'), 'role="form" id="form"') ?>
	<div class="row">
		<div class="col-md-8">
			<?= generate_sub_headline($title, 'fa-folder', '', FALSE) ?>
			<hr class="visible-xs"/>
		</div>
			...
			<table class="table table-striped table-hover">
					...
					<tr>
						...
						<td style="width: 20%" class="text-right">
							<?php if (!$disable_sql_category_count): ?>
								<a href="<?= admin_url(TBL_KB_ARTICLES . '/view?m-category_id=' . $v[ 'category_id' ]) ?>"
							...
							<?php if ($v[ 'parent_id' ] > 0): ?>
								<a href="<?= admin_url(TBL_KB_CATEGORIES . '/view?p-parent_id=' . $v[ 'top_parent' ]) ?>"
								   class="tip btn btn-default" data-toggle="tooltip" data-placement="bottom"
								   title="<?= lang('go_up_level') ?>"><?= i('fa fa-arrow-up') ?></a>
							<?php endif; ?>
							<a href="<?= admin_url(TBL_KB_CATEGORIES . '/view?p-parent_id=' . $v[ 'category_id' ]) ?>"
							   class="tip btn btn-default data-toggle=" tooltip" data-placement="bottom"
							title="<?= lang('view_sub_categories') ?>"><?= i('fa fa-arrow-down') ?></a>
							<a href="<?= admin_url(TBL_KB_ARTICLES . '/create/' . $v['category_id']) ?>"
							   class="tip btn btn-info <?= is_disabled('create') ?>"  data-toggle="tooltip"
							   data-placement="bottom" title="<?= lang('create_kb_article') ?>">
								<?= i('fa fa-file-text-o') ?></a>
							<a href="<?= admin_url(CONTROLLER_CLASS . '/update/' . $v[ 'category_id' ]) ?>"
							   class="btn btn-default" title="<?= lang('edit') ?>"><?= i('fa fa-pencil') ?></a>
							<?php if ($v[ 'category_id' ] != $default_kb_category_id): ?>
				...
				<tr>
                    ...
                    <button class="btn btn-primary <?= is_disabled('update', TRUE) ?>"
                        ...
                        <a href="<?=admin_url(CONTROLLER_CLASS . '/view?p-parent_id=0')?>" class="btn btn-default">
                            <?=lang('view_top_level_categories')?>
                        </a>
                        <a href="<?=admin_url(CONTROLLER_CLASS . '/view')?>" class="btn btn-info">
