...
<div class="row">
	...
	<div class="col-md-7 text-right">
		<?php if (CONTROLLER_FUNCTION == 'update'): ?>
			...
			<?php if ($row[ 'category_id' ] != $default_kb_category_id): ?>
...
<div class="box-info">
	...
	<div class="tab-content">
				...
				<?php foreach ($row[ 'lang' ] as $v): ?>
					<div
						...
						<div class="form-group">
							...
							<div class="col-md-5">
								<?= form_input('lang[' . $v[ 'language_id' ] . '][category_name]', set_value('category_name', $v[ 'category_name' ]), 'class="' . css_error('category_name') . ' form-control"') ?>
							</div>
						</div>
						<hr/>
						<div class="form-group">
		...
		<div class="tab-pane fade in" id="config">
			...
			<hr/>
			<div class="form-group">
				<?= lang('parent_category', 'parent_id', 'class="col-md-3 control-label"') ?>
...
<script>
	...
	$("#parent_id").select2({
		ajax: {
			...
			<?php else: ?>
			url: '<?=admin_url(TBL_KB_CATEGORIES . '/search/ajax/category_name/' . $id)?>',
			<?php endif; ?>
			dataType: 'json',
			delay: 250,
			data: function (params) {
				return {
					category_id: params.term, // search term
					page: params.page
				};
			},
