...
<div class="row">
	...
	<div class="col-md-7 text-right">
		...
		<a href="<?= admin_url(CONTROLLER_CLASS) ?>" class="btn btn-primary"><?= i('fa fa-search') ?> <span
				class="hidden-xs"><?= lang('view_faq') ?></span></a>
		<?php if (CONTROLLER_FUNCTION == 'update'): ?>
	...
	<div class="tab-content">
		...
		<div class="tab-pane fade in <?php if ($v[ 'language_id' ] == $sts_site_default_language): ?> active <?php endif; ?>"
			...
			<div class="form-group">
				<label for="question"
				       class="col-sm-3 control-label"><?= lang('question') ?></label>

				<div class="col-sm-5">
					<?= form_input('lang[' . $v['language_id'] . '][question]', set_value('question', $v['question']), 'class="' . css_error('question') . ' form-control"') ?>
				</div>
			</div>
			<hr/>
			<div class="form-group">
				<label for="answer"
				       class="col-sm-3 control-label"><?= lang('answer') ?></label>

				<div class="col-sm-7">
					<?= form_textarea('lang[' . $v['language_id'] . '][answer]', set_value('answer', $v['answer'], FALSE), 'class="editor ' . css_error('answer') . ' form-control"') ?>
				</div>
			</div>
			<hr/>
		</div>
