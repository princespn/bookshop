 ...
 * @filesource
 */
class Print_checks_model extends Affiliate_payments_model
{
	...
	{
		$config = array(
			'settings_key'	=>	'module_affiliate_payments_print_checks_use_date_range',
			'settings_value'	=>	'0',
		...
		$config = array(
			'settings_key'	=>	'module_affiliate_payments_print_checks_start_date',
		...
		$config = array(
			'settings_key'	=>	'module_affiliate_payments_print_checks_end_date',
			'settings_value'	=>	get_time('', TRUE),
			'settings_module'	=>	'affiliate_payments',
			'settings_type'	=>	'text',
			'settings_group'	=>	$id,
		...
		$config = array(
			'settings_key'	=>	'module_affiliate_payments_print_checks_exclude_minimum',
			'settings_value'	=>	'0',
			'settings_module'	=>	'affiliate_payments',
			'settings_type'	=>	'dropdown',
			'settings_group'	=>	$id,
		...
		$config = array(
			'settings_key'	=>	'module_affiliate_payments_print_checks_total_rows',
			'settings_value'	=>	'100',
			'settings_module'	=>	'affiliate_payments',
			'settings_type'	=>	'text',
			'settings_group'	=>	$id,
		...
		$config = array(
			'settings_key'	=>	'module_affiliate_payments_print_checks_payment_details',
	...
	{
		$a = $this->load->view('admin/tpl_admin_print_checks_header', '', true);

		foreach($data['select'] as $v)
		{
			$a .= $this->load->view('admin/tpl_admin_print_checks', $data['member'][$v], true);
	...
	public function run_query()
	{
		...
		if (config_enabled('module_affiliate_payments_print_checks_use_date_range'))
		{
			$sql .=	' AND (p.date > \'' . config_option('module_affiliate_payments_print_checks_start_date') . '\'
						AND p.date < \'' . config_option('module_affiliate_payments_print_checks_end_date') . '\')';
		}
		...
		$sql .= ' GROUP by p.member_id
			...
			. config_option('module_view_function_sort_order') . '
					 LIMIT ' . config_option('module_affiliate_payments_print_checks_total_rows');
...
/* End of file Print_checks_model.php */
/* Location: ./modules/affiliate_payments/print_checks/models/Print_checks_model.php */