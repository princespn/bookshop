		...
		<div class="<?= mobile_view('hidden-xs') ?>">
			<table class="table table-striped table-hover">
				...
				<tr>
					...
					<th class="text-center hidden-xs"><?= tb_header('dwolla_email', '', FALSE) ?></th>
				...
				<?php foreach ($rows[ 'values' ] as $v): ?>
					<?php if ($v[ 'total_amount' ] < config_option('sts_affiliate_min_payment_amount')): ?>
						<?php if (config_enabled('module_affiliate_payments_dwolla_mass_payment_exclude_minimum')): ?>
							<?php continue; ?>
						<?php endif; ?>
					<?php endif; ?>
					<tr>
						<td class="text-center">
							<?php if (!empty($v[ 'dwolla_id' ])): ?>
						...
						<td class="text-center">
							<?= $v[ 'payment_name' ] ?>
							<?= form_hidden('member[' . $v[ 'member_id' ] . '][payment_name]', $v[ 'payment_name' ]) ?>
						</td>
						<td class="text-center">
							<?= $v[ 'dwolla_id' ] ?>
							<?= form_hidden('member[' . $v[ 'member_id' ] . '][dwolla_id]', $v[ 'dwolla_id' ]) ?>
						</td>
						<td class="text-center">
						...
						<td class="text-center">
							...
							<?php endif; ?>
							<?= format_amount($v[ 'total_amount' ], FALSE) ?> <?=config_option('module_affiliate_payments_dwolla_mass_payment_currency')?>
							<?= form_hidden('member[' . $v[ 'member_id' ] . '][total_amount]', $v[ 'total_amount' ]) ?>
						</td>
						<td class="text-right">
							<?php if (!empty($v[ 'dwolla_id' ])): ?>
							<a href="<?=admin_url(CONTROLLER_CLASS . '/direct_pay/' . $id . '/?type=' . urlencode($row['module']['module_name']) . '&member_id=' . $v['member_id'] . '&id=' . urlencode($v[ 'dwolla_id' ])) ?>&amount=<?= $v[ 'total_amount' ] ?>&name=<?=urlencode($v['payment_name'])?>"><img src="<?= base_url('images/modules/pay_dwolla.png') ?>"/></a>
							<?php endif; ?>
						</td>
					</tr>
				<?php endforeach ?>
