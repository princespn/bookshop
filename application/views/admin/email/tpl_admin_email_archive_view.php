    ...
    <div class="row">
        <div class="col-md-8">
            <div class="input-group text-capitalize">
                <?= generate_sub_headline('archived_email', 'fa-envelope', $rows['total']) ?>
            </div>
        </div>
        <div class="col-md-4 text-right">
            ...
            <a data-href="<?= admin_url(CONTROLLER_CLASS . '/reset/') ?>" data-toggle="modal"
               ...
               class="md-trigger btn btn-danger <?= is_disabled('delete') ?>"><?= i('fa fa-trash-o') ?> <span
                    class="hidden-xs"><?= lang('empty_archive') ?></span></a>
                ...
                <?php foreach ($rows['values'] as $v): ?>
                    <tr>
                        ...
                        <td style="width: 15%" class="text-center hidden-xs"><h5><?= $v['recipient_name'] ?></h5><span
                                class="label label-primary"><?= $v['recipient_email'] ?></span></td>
                        <td>
                            <div class="overflow">
                                <h5><?= $v['subject'] ?></h5>
                    ...
                    <td colspan="3">
                        <?php if ($this->sec->check_admin_permissions(CONTROLLER_CLASS, 'delete') == true): ?>
                            <div class="input-group text-capitalize">
                                ...
                                <?= form_dropdown('change-status', options('email_archive', 'delete'), '', 'id="change-status" class="form-control"') ?>
