 ...
 * @filesource
 */
class Import_members_model extends Data_import_model
{
	...
	{
		$config = array(
			'settings_key'        => 'module_data_import_members_use_server_path',
			'settings_value'      => '0',
		...
		$config = array(
			'settings_key'        => 'module_data_import_members_server_file_path',
		...
		$config = array(
			'settings_key'        => 'module_data_import_members_first_row_column_names',
			'settings_value'      => '1',
			'settings_module'     => 'data_import',
			'settings_type'       => 'dropdown',
			'settings_group'      => $id,
	...
	public function validate_import_module($data = array())
	{
		...
		if (!file_exists($data['module_data_import_members_server_file_path']))
		{
	...
	public function generate_fields()
	{
		...
		$csv = array_map('read_csv', file(config_option('module_data_import_members_server_file_path')));
	...
	public function generate_field_names()
	{
		...
		foreach ($this->config->item('module_import_tables') as $v)
		{
			...
			foreach ($fields as $f)
			{
				if (config_option('module_data_import_members_generate_new_ids') == 1)
				{
					if ($f == 'member_id')
					{
						continue;
	...
	public function do_import($data = array())
	{
		...
		//let's read the file now...
		$lines = array_map('str_getcsv', file(config_option('module_data_import_members_server_file_path')));
		...
		if (!empty($lines))
		{
			...
			{
				//remove the colum names first...
				if ($a == 0)
				{
					if (config_option('module_data_import_members_first_row_column_names') == 1)
					{
						continue;
					}
				}
	...
	{
		//add the members table first and get the member_id
		$mem = $this->mem->add_member($data[TBL_MEMBERS]);

		foreach ($data as $k => $v)
		{
			$v['member_id'] = $mem['member_id'];

			switch ($k)
			{
				case TBL_MEMBERS_ADDRESSES:

					$this->mem->add_address($v);

					break;

				case TBL_MEMBERS_PASSWORDS:

					//add the members password
					$p = empty($v['password']) ? random_string('alnum', 8) : $v['password'];
					$this->mem->add_member_password(array('member_id' => (int)$v['member_id'],
					                                      'password'  => $p)
					);

					break;

				case TBL_MEMBERS_PROFILES:

					//add profile
					$this->mem->add_profile($v);

					break;

				case TBL_MEMBERS_AFFILIATE_GROUPS:

					//add affiliate group
					$p = empty($v['group_id']) ? config_option('sts_affiliate_default_registration_group') : $v['group_id'];
					$this->mem->add_group(TBL_MEMBERS_AFFILIATE_GROUPS, array('member_id' => (int)$v['member_id'],
					                                                          'group_id'  => $p,
					));

					break;

				case TBL_MEMBERS_BLOG_GROUPS:

					//add contact group
					$p = empty($v['group_id']) ? config_option('sts_members_default_blog_group') : $v['group_id'];
					$this->mem->add_group(TBL_MEMBERS_BLOG_GROUPS, array('member_id' => (int)$v['member_id'],
					                                                     'group_id'  => $p,
					));

					break;

				case TBL_MEMBERS_DISCOUNT_GROUPS:

					//add discount group
					$p = empty($v['group_id']) ? config_option('sts_members_default_discount_group') : $v['group_id'];
					$this->mem->add_group(TBL_MEMBERS_DISCOUNT_GROUPS, array('member_id' => (int)$v['member_id'],
					                                                         'group_id'  => $p,
					));
				...
				case TBL_MEMBERS_PERMISSIONS:
					...
					//permissions
					$this->mem->add_permissions($v);

					break;

				case TBL_MEMBERS_SPONSORS:

					//set sponsor if any
					$p = !empty($v['sponsor_id']) ? (int)$v['sponsor_id'] : '0';
					$this->mem->add_sponsor(array('member_id'           => (int)$v['member_id'],
					                              'sponsor_id'          => $p,
					                              'original_sponsor_id' => $p,
					));

					break;

				case TBL_MEMBERS_ALERTS:

					//addd member alerts
					$this->mem->add_alerts($v);

					break;

				case TBL_MEMBERS_EMAIL_MAILING_LIST:

					if (!empty($v['list_id']))
					{
						$vars = $data[TBL_MEMBERS];
						$vars['member_id'] = $v['member_id'];
						$vars['mailing_lists'] = $v['list_id'];

						$this->mem->update_member_lists($vars);
					}
					...
					foreach ($fields[$k] as $f)
					{
						...
						switch ($f->name)
						{
							case 'member_id':
								...
								if (config_option('module_data_import_members_generate_new_ids') == '0')
								{
									$rule .= '|check_member_id';

									$custom_lang = array(
										'check_member_id' => '%s ' . $v['member_id'] . ' ' . lang('already_exists'),
									);
								}
							...
							case 'primary_email':
								...
								$rule = array_merge($r, $b);

								if (!empty($v['primary_email']))
								{
									$custom_lang = array(
										'check_email' => '%s ' . $v['primary_email'] . ' ' . lang('already_exists'),
									);
								}
							...
							case 'username':
								...
								if (!empty($v['username']))
								{
									...
									$custom_lang = array(
										'check_username' => '%s ' . $v['username'] . ' ' . lang('already_exists'),
									);
								}
...
/* End of file Import_members_model.php */
/* Location: ./application/modules/data_import/members/models/Import_members_model.php */