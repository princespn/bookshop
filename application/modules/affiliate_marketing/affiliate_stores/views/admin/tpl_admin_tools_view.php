...
<div class="row">
	<div class="col-md-8">
		<div class="input-group text-capitalize">
			<?= generate_sub_headline('affiliate_stores', 'fa-list', $rows['total']) ?>
		</div>
	</div>
	<div class="col-md-4 text-right">
		...
		<a href="<?= admin_url(CONTROLLER_CLASS . '/create/' . $module_row['module']['module_id']) ?>"
		   class="btn btn-primary <?= is_disabled('create') ?>"><?= i('fa fa-plus') ?> <span
				class="hidden-xs"><?= lang('add_store') ?></span></a>
...
<div class="box-info">
	<div class="row">
		...
		<div class="col-md-7"><?= tb_header('store_name', 'store_name') ?></div>
		<div class="col-md-2 text-center"><?= tb_header('affiliate', 'username') ?></div>
	...
	<div>
		...
		<div>
			<div class="row">
				...
				<div class="col-md-7"><h5><?= $v['name'] ?></h5></div>
				<div class="col-md-2 text-center"><h5><a href="<?=admin_url(TBL_MEMBERS . '/update/' . $v['member_id'])?>"><?=$v['username']?></a></h5></div>
				<div class="col-md-2 text-right">
					<a href="<?= site_url('shop/'.  $v['username']) ?>" target="_blank"
					   class="btn btn-default" title="<?= lang('edit') ?>"><?= i('fa fa-external-link') ?></a>
