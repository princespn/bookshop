...
<hr/>
<div class="box-info">
	<h3><?= lang('queueing_your_emails') ?>. <?= lang('please_wait') ?>....</h3>
	<hr/>
	<div class="jumbotron dashed text-center">
		...
		<div id="continue" class="hidden text-right">
			<hr />
			<a href="<?=admin_url('email_queue/view')?>" class="btn btn-info"><?=i('fa fa-search')?> <?=lang('view_email_queue')?></a>
			<a href="<?=admin_url('email_mailing_lists/view')?>" class="btn btn-primary"><?=i('fa fa-caret-right')?> <?=lang('continue')?></a>
		</div>
...
<script>
	...
	function submit_form(qid) {
		$.ajax({
			url: '<?=admin_url('email_send/queue_mass_email/')?>',
			type: 'POST',
			dataType: 'json',
			data: $('#form').serialize(),
			success: function (response) {
				if (response.type == 'continue') {
					...
					$("#success-msg").html('<h3>'+ response.queued + ' <?=lang('emails_queued_for_sending')?>!</h3>');
