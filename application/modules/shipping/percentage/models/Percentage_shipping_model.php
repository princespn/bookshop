 ...
 * @filesource
 */
class Percentage_shipping_model extends Shipping_model
{
	...
	public function install($id = '')
	{
		...
		$config = array(
			'settings_key'        => 'module_shipping_percentage_charge_shipping_tax',
			'settings_value'      => '0',
		...
		$config = array(
			'settings_key'        => 'module_shipping_percentage_tax_class',
			'settings_value'      => '1',
			'settings_module'     => 'shipping',
			'settings_type'       => 'dropdown',
			'settings_group'      => $id,
		...
		$config = array(
			'settings_key'        => 'module_shipping_percentage_quantity_type',
			'settings_value'      => '1',
			'settings_module'     => 'shipping',
			'settings_type'       => 'dropdown',
			'settings_group'      => $id,
	...
	public function generate_rates($data = array(), $cart = array())
	{
		...
		if (!empty($zone_rate))
		{
			...
			$zone_rate['shipping_amount'] = ($cart['totals']['sub_total_discounts'] + $cart['totals']['coupons']) * show_percent($zone_rate['amount']);

			//add some taxes and handling...
			$zone_rate['shipping_taxes'] = calc_shipping_taxes($zone_rate['shipping_amount'], 'percentage');
			$zone_rate['shipping_total'] = shipping_totals($zone_rate);
		}

		return empty($zone_rate['shipping_total']) ? FALSE : array($zone_rate);
	}
...
/* End of file Percentage_shipping_model.php */
/* Location: ./application/modules/shipping/percentage/models/Percentage_shipping_model.php */